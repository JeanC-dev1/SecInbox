...
def main():
    ...
    if validar_url(input_text):
        resultado = analisar_texto(input_text, tipo="url")
    elif validar_email(input_text):
        resultado = analisar_texto(input_text, tipo="email")
    else:
        print("[SecInbox] Entrada inválida. Forneça um link ou e-mail.")
        sys.exit(1)

    # Exibe o resultado
    if resultado["suspicious"]:
        print(f"[Alerta] Possível ameaça detectada: {resultado['reason']}")
    else:
